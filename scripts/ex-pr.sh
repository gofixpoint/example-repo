...
amika-local sandbox create --git --secret env:ANTHROPIC_API_KEY --name testing-claude-api
...
amika-local sandbox ssh testing-claude-api << EOF 2>/dev/null
...
amika-local sandbox ssh -t testing-claude-api << EOF 2>/dev/null
